    ...
    <div style={{ minHeight: '100vh', background: '#F9F7F4' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          {entries.length === 0 ? (
            ...
            <Link key={entry.id} href={`/${entry.username}`} style={{ textDecoration: 'none' }}>
              <div style={{
                background: '#FFFFFF', border: '1px solid #E8E4DF', borderRadius: '12px',
                padding: '1rem 1.25rem', display: 'flex', alignItems: 'center', gap: '1rem',
                transition: 'border-color 0.15s, transform 0.15s',
              }}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = '#1A1714'; (e.currentTarget as HTMLElement).style.transform = 'translateY(-1px)' }}
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = '#E8E4DF'; (e.currentTarget as HTMLElement).style.transform = 'translateY(0)' }}
              >
                <div style={{ width: 32, height: 32, background: i === 0 ? '#1A1714' : '#F3F0ED', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <span style={{ fontSize: '0.75rem', fontWeight: 700, color: i === 0 ? '#fff' : '#6B6560' }}>#{entry.rank}</span>
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                    <span style={{ fontWeight: 600, color: '#1A1714', fontSize: '0.9rem' }}>{entry.full_name}</span>
                    <span style={{ fontSize: '0.75rem', color: '#9C9590' }}>@{entry.username}</span>
                    <span style={{ fontSize: '0.7rem', background: '#EBF2EA', color: '#2D5A27', padding: '0.15rem 0.5rem', borderRadius: '100px', fontWeight: 600 }}>✓ Verified</span>
                    {entry.is_featured && <span style={{ fontSize: '0.7rem', background: '#1A1714', color: '#fff', padding: '0.15rem 0.5rem', borderRadius: '100px', fontWeight: 600 }}>Featured</span>}
                  </div>
                  <div style={{ fontSize: '0.78rem', color: '#9C9590', marginTop: '0.125rem' }}>
                ...
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  <div style={{ fontFamily: 'DM Serif Display, serif', fontSize: '1.1rem', color: '#1A1714' }}>{formatIDRShort(entry.verified_mrr)}</div>
                  <div style={{ fontSize: '0.72rem', color: '#9C9590', fontWeight: 500 }}>MRR</div>
                </div>
              </div>
            </Link>
          ))}
      ...
      <section style={{ background: '#1A1714', padding: '5rem 2rem', textAlign: 'center' }}>
        <div style={{ maxWidth: 520, margin: '0 auto' }}>
          <h2 style={{ fontFamily: 'DM Serif Display, serif', fontSize: '2rem', color: '#FFFFFF', marginBottom: '1rem' }}>
            Revenue kamu nyata?<br />Buktikan.
          </h2>
          <p style={{ color: '#9C9590', fontSize: '0.95rem', lineHeight: 1.7, marginBottom: '2rem' }}>
            Submit API key Mayar, Scalev, atau Lynk kamu. Kami verifikasi langsung dari sumbernya.
          </p>
